...
|---|---|---|---|---|
| Multi-tenant isolation (RLS + GUC + FORCE) | 🟢 DONE |  46 cột `tenant_id`, RLS NULLIF-hardened fail-closed, e2e cross-tenant đỏ-khi-sai | ADR-040, V011-026 |
| RLS 3-pool runtime (icp_app NOBYPASSRLS) | 🟢 DONE |  Tách pool app/migrate/privileged — chống bypass | ADR-040 Am 2026-06-17 |
| Auth (JWT + session + refresh + switch-tenant) | 🟢 DONE |  10 route auth; thiếu MFA/SSO/lockout (xem §3 auth-hardening) | S-03, ADR-046 |
| RBAC actor taxonomy (buyer/merchant/supplier/staff/admin) | 🟢 DONE |  `tenants.type` + `tenant_memberships` + role-guard; thiếu role auditor/support | ADR-062, V034 |
| Idempotency (Redis SETNX + response cache) | 🟢 DONE |  Chống double-charge/replay; 2-lớp payment_callbacks + CDC inbox | ADR-004 |
| Event backbone (CDC Debezium + Redpanda + outbox) | 🟢 DONE | Outbox same-txn, CDC slot-aware, exactly-once inbox | ADR-006/058, V017-020 |
| Observability (OTel logs/traces/metrics) | 🟢 DONE |  170+ log type, trace xuyên 3-service; thiếu vài histogram (Vespa/Kafka) | ADR-011 |
| Contract-first FE↔BE (OpenAPI codegen + Zod SOT) | 🟢 DONE | CI drift-gate, generated client | ADR-017 |
| CI/CD hard-gate (lint/test/typecheck/coverage/drift) | 🟢 DONE |  8 job, coverage floor ≥70%, facts-drift guard | S-P0-03/T01 |
| HA/scale (pgbouncer, Vespa cluster, multi-server) | 🔵 PLANNED |  Single-node hiện tại; pooler + Vespa HA = scale-phase | ADR-067, BACKLOG P2 |
...
|---|---|---|---|---|
| Storefront + PDP (trang chi tiết SP, buyer-scoped) | 🟢 DONE |  Storefront read API field-projected + variant selector | ADR-074, S-INT05/T02a-b |
| Giỏ hàng (add/qty/remove/promo/variant) | 🟢 DONE | | Composite line-key variant, tax, honor-lower pricing | S-INT05, ADR-073 |
| Lưu để sau (save-for-later) | 🟢 DONE |  Bảng riêng RLS, move-to-cart | V055, ADR-073 |
| Checkout + sổ địa chỉ + phí ship | 🟢 DONE |  Address-book, shipping rate, COD/bank | S-INT06, ADR-070 |
| Theo dõi đơn realtime (SSE) | 🟢 DONE |  Order-status stream | S-INT06/T05 |
| Tìm bằng text (AI hybrid Vespa) | 🟡 PARTIAL |  BM25+CLIP chạy; **thiếu facets/sort/pagination/filter** | Intent-03 |
| Tìm/mua bằng giọng nói | 🟡 PARTIAL |  STT+parse chạy; **thiếu review-state, OOS-substitute, TTS-confirm** | Intent-02 |
| Gợi ý bằng ảnh (visual reco) | 🟡 PARTIAL |  Vision + 3-signal blend; **empty-state dead-end, thiếu grid** | Intent-04 |
| Duyệt theo danh mục / browse | 🟡 PARTIAL | Có category; thiếu trang browse đầy đủ + facet sidebar | ADR-071 |
| Autocomplete / gợi-ý-gõ | 🟠 NEEDED | 0% | Amazon/Shopee baseline | — |
| Đánh giá & xếp hạng (reviews/ratings) | 🟠 NEEDED | **Cốt lõi niềm tin mua hàng** — chưa có | — |
| Hỏi-đáp sản phẩm (Q&A) | 🟠 NEEDED |  — | — |
| Yêu cầu trả hàng / hoàn tiền (buyer) | 🟠 NEEDED |  Returns/refund request flow | — |
| Mua lại nhanh (reorder) | 🟠 NEEDED |  — | — |
| Wishlist / so sánh / vừa-xem | 🟠 NEEDED |  save-for-later ≠ wishlist công khai | — |
| Thông báo đơn (push/SMS/Zalo) | 🟠 NEEDED |  VN kỳ vọng tin nhắn cập nhật | xem §7,§10 |
| Đánh giá sau mua + ảnh review | 🟠 NEEDED |  UGC = data + trust | — |
